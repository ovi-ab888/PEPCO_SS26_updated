...
def format_product_translations(product_name, translation_row,
    ...
    # English: composition first if provided
    en_text = str(translation_row['EN']) if hasattr(translation_row, "get") and pd.notna(translation_row.get('EN')) else (
        translation_row['EN'] if isinstance(translation_row, dict) and 'EN' in translation_row else product_name
    )
    if composition_texts and composition_texts.get('EN'):
        formatted.append(f"|EN| {composition_texts['EN']} — {en_text}")
    else:
        formatted.append(f"|EN| {en_text}")

    combined_languages = {
        'ES': f"{translation_row['ES']} / {translation_row['ES_CA']}"
              if (hasattr(translation_row, "get") and pd.notna(translation_row.get('ES_CA')))
              else (translation_row['ES'] if (hasattr(translation_row, "get") and pd.notna(translation_row.get('ES'))) else (translation_row.get('ES', product_name) if isinstance(translation_row, dict) else product_name))
    } if (hasattr(translation_row, "get") and ('ES' in translation_row and pd.notna(translation_row.get('ES')))) or (isinstance(translation_row, dict) and 'ES' in translation_row) else {}
    ...
    for lang in language_order:
        if lang in combined_languages:
            base_text = combined_languages[lang]
        elif hasattr(translation_row, "get") and pd.notna(translation_row.get(lang)):
            base_text = translation_row[lang]
        elif isinstance(translation_row, dict) and lang in translation_row:
            base_text = translation_row[lang]
        else:
            base_text = product_name

        # Put composition first for AL/BG/MK/RS (and any others we have)
        if composition_texts and composition_texts.get(lang):
            text = f"{composition_texts[lang]} — {base_text}"
        else:
            text = base_text
...
if not material_translations_df.empty:
    ...
    # Percentage inputs per selected material
    composition_pairs = []
    composition_texts = {}
    if selected_materials:
        st.caption("Enter composition percentages (must sum to 100%)")
        cols = st.columns(min(4, len(selected_materials)))
        for i_m, m in enumerate(selected_materials):
            with cols[i_m % len(cols)]:
                pct = st.number_input(f"% {m}", min_value=0, max_value=100, value=0, step=1, key=f"pct_{m}")
        ...
        # Helper to translate a material name to a given language
        ...
        # Build composition strings: EN and selected languages
        en_parts = [f"{pct}% {mat.lower()}" for mat, pct in composition_pairs if pct > 0]
        composition_texts['EN'] = " ".join(en_parts).strip()
        for lang in ['AL', 'BG', 'MK', 'RS']:
            parts = [f"{pct}% {get_translation(mat, lang)}" for mat, pct in composition_pairs if pct > 0]
            composition_texts[lang] = " ".join(parts).strip()

    # Cotton flag: Y only if 100% cotton
    cotton_pct = 0
    for mat, pct in (composition_pairs or []):
        if str(mat).strip().lower() == "cotton":
            cotton_pct += pct
    cotton_value = "Y" if cotton_pct == 100 else ""

    # Keep material_trans_dict for backward-compat (not used if composition_texts exists)
    material_trans_dict = {}
    if selected_materials:
        for lang in ['AL', 'BG', 'MK', 'RS']:
            trans_list = []
            ...
            if pln_price is not None:
                ...
                if currency_values:
                    ...
                    final_cols = [
                        ...
                        "Colour_SKU", "Style_Merch_Season", "Batch", "barcode", "washing_code", "Composition",
...
st.caption("This app developed by Ovi")
                # Save composition string for export
                df['Composition'] = (composition_texts.get('EN') if (locals().get('composition_texts')) else '')
